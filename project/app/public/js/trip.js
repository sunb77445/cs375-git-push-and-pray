...
let trip_name = document.getElementById("tripName");
...
// Fetches full details for this trip, renders every section, and sets
// currentTripRole/tripDetails so the edit/add controls know what they're
// allowed to do.
async function getTripDetails(){
    try {
        ...
        if (trip_name) trip_name.textContent = data.details[0].name;
...
if (addHotelButton) {
    addHotelButton.addEventListener("click", async () => {
        const wantsManualEntry = confirm(
            "Click OK to manually enter a hotel's details.\n" +
            "Click Cancel to browse live hotel results and pick one instead."
        );

        if (wantsManualEntry) {
            await manualAddHotel();
        } else {
            browseHotelsForEdit(null);
        }
    });
}

async function manualAddHotel() {
    ...
    try {
        const response = await fetch("/api/hotels/save", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ tripId: trip_id, hotel: name, price, guests })
        });
...
if (addFlightButton) {
    addFlightButton.addEventListener("click", async () => {
        const wantsManualEntry = confirm(
            "Click OK to manually enter a flight's details.\n" +
            "Click Cancel to browse live flight results and pick one instead."
        );

        if (wantsManualEntry) {
            await manualAddFlight();
        } else {
            browseFlightsForEdit(null);
        }
    });
}

async function manualAddFlight() {
    ...
    try {
        const response = await fetch("/api/flights/save", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ tripId: trip_id, route, airline, price })
        });
        ...
        foodData.features.forEach(place => {
            ...
            `, async () => {
                ...
                const saveResponse = await fetch(endpoint, {
                    method,
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ tripId: trip_id, name, address, website })
                });
...
if (addRestaurantButton) {
    addRestaurantButton.addEventListener("click", async () => {
        const wantsManualEntry = confirm(
            "Click OK to manually enter a restaurant's details.\n" +
            "Click Cancel to browse live restaurant results and pick one instead."
        );

        if (wantsManualEntry) {
            await manualAddRestaurant();
        } else {
            browseRestaurantsForEdit(null);
        }
    });
}

async function manualAddRestaurant() {
