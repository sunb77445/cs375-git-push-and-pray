...
tabs.forEach(tab => {
    tab.addEventListener("click", (event) =>{
        ...
        if(page.id == "selected-results"){
             ...
             // Render the selected restaurant (selectedRestaurant comes from restaurant.js)
             foodSelection.replaceChildren();
             if (typeof selectedRestaurant !== "undefined" && selectedRestaurant) {
                 const restaurantEl = document.createElement("div");
                 restaurantEl.innerHTML = `
                    <h2>${selectedRestaurant.name}</h2>
                    <p>${selectedRestaurant.address || ""}</p>
                 `;
                 foodSelection.appendChild(restaurantEl);
             } else {
                 foodSelection.textContent = "No restaurant selected";
             }
